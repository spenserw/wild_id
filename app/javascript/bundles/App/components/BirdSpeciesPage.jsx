...
const BirdSpeciesPage = () => {
  ...
  const colorForCounty = (fipsCode) => {
    ...
    const [match, stateCode, countyCode] = fipsCode.match(/(\d\d)(\d\d\d)/);

    const seasonalPresence = Object.keys(seasonToColor)
      .map((season) =>
        distribution[season]
          && Object.keys(distribution[season]).includes(stateCode)
          && Object.keys(distribution[season][stateCode]).includes(countyCode)
          ? season
          : null
      )
      .filter(_ => _ != null);
  ...
  const renderGeography = (geo) => {
    ...
    const stroke = color.includes("url")
      ? seasonToColor[color.match(/url\(#\S+-(\S)/)[1]]
      : color;
    return (
      <Geography
        key={geo.rsmKey}
  ...
  const mixedPresencePatterns = () => {
    ...
    for (let i = 0; i < Object.keys(seasonToColor).length; i++) {
      for (let j = i + 1; j < Object.keys(seasonToColor).length; j++) {
        ...
        patterns.push(
          generatePatternSvgElement(
            `${firstKey}-${secondKey}`,
            seasonToColor[firstKey],
            seasonToColor[secondKey]
          )
        );

        // Collision of 3 zones can really happen!! See example `Buteo regalis`! I wonder if all four could?
  ...
  const renderGalleryImages = () => {
    return concat(
      speciesInfo.primary_reference_image_id,
      speciesInfo.gallery_reference_image_ids
    ).map((uuid) => {
      const srcPath = get_reference_image_src(uuid, "l");
      return { src: srcPath };
    });
  };

  // Don't render until species info is loaded
  if (Object.keys(speciesInfo).length == 0) return <div />;

  return (
    <BirdSpeciesPageContainer>
      <BirdSpeciesGalleryContainer>
        <Carousel
          images={renderGalleryImages()}
      ...
      <BirdSpeciesPageInfoConatiner>
        <BirdSpeciesPageInfoName>
          <Space>
            {speciesInfo.common_names[0]}(<i>{speciesInfo.scientific_name}</i>)
          </Space>
        </BirdSpeciesPageInfoName>
      ...
      <BirdSpeciesPageMapContainer>
        <ComposableMap
          ...
          projectionConfig={{ center: mapCenter }}
        >
